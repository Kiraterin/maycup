...
TEST_CASE(ast_ctor_illegal_arg) {
    ...
    ASSERT_EQ(maycup_ast_ctor(NULL, &root), MAYCUP_RESULT_ILLEGAL_ARGUMENT,
              fail);
    ASSERT_EQ(maycup_ast_ctor(&ast, NULL), MAYCUP_RESULT_ILLEGAL_ARGUMENT,
              fail);
...
TEST_CASE(ast_insert_normal) {
    ...
    ssize_t node[5];
    ASSERT_OK(
        maycup_insert_astnode(&node[0], &ast, root, MAYCUP_ASTNODE_TYPE_NONE),
        fail);
    ASSERT_OK(maycup_insert_astnode(&node[1], &ast, node[0],
    ...
    ASSERT_OK(maycup_insert_astnode(&node[2], &ast, node[0],
    ...
    ASSERT_OK(maycup_insert_astnode(&node[3], &ast, node[0],
                                    MAYCUP_ASTNODE_TYPE_NONE),
              fail);
    ...
    ASSERT_OK(maycup_insert_astnode(&node[4], &ast, node[2],
                                    MAYCUP_ASTNODE_TYPE_NONE),
              fail);
...
TEST_CASE(ast_insert_realloc_fail) {
    ...
    ASSERT_OK(maycup_ast_ctor(&ast, &root), fail);
    for (size_t i = 0; i < cnt_1st; ++i) {
        res = maycup_insert_astnode(&node_1st[i], &ast, root,
                                    MAYCUP_ASTNODE_TYPE_NONE);
...
TEST_CASE(ast_delete_illegal_arg) {
    ...
    ASSERT_EQ(maycup_delete_astnode(NULL, root), MAYCUP_RESULT_ILLEGAL_ARGUMENT,
              fail);
    ASSERT_EQ(maycup_delete_astnode(&ast, 0), MAYCUP_RESULT_ILLEGAL_ARGUMENT,
              fail);
...
TEST_CASE(astnode_text_ctor_normal) {
    ...
    ASSERT_OK(maycup_astnode_data_text_ctor(&andt1, text1,
...
TEST_CASE(astnode_text_ctor_illegal_arg) {
    ...
    ASSERT_EQ(maycup_astnode_data_text_ctor(NULL, text, MAYCUP_TEXTSTYLE_PLAIN,
                                            false),
              MAYCUP_RESULT_ILLEGAL_ARGUMENT, fail);
    ASSERT_EQ(maycup_astnode_data_text_ctor(&andt, NULL, MAYCUP_TEXTSTYLE_PLAIN,
                                            false),
              MAYCUP_RESULT_ILLEGAL_ARGUMENT, fail);
...
TEST_CASE(astnode_text_dtor_normal) {
    ...
    ASSERT_OK(maycup_astnode_data_text_ctor(&andt, text, MAYCUP_TEXTSTYLE_PLAIN,
                                            false),
              fail);
...
TEST_CASE(astnode_text_dtor_illegal_arg) {
    ASSERT_EQ(maycup_astnode_data_text_dtor(NULL),
              MAYCUP_RESULT_ILLEGAL_ARGUMENT, fail);
    return TEST_RESULT_PASS;
fail:
    return TEST_RESULT_FAIL;
...
TEST_CASE(ast_module_common) {
    ...
    ASSERT_OK(maycup_insert_astnode(&node[0], &ast, root,
                                    MAYCUP_ASTNODE_TYPE_HEADING),
              fail);
    ...
    ASSERT_OK(maycup_astnode_data_text_ctor(&ast.data[node[1]].text,
                                            ...
                                            MAYCUP_TEXTSTYLE_PLAIN, false),
              fail);
    ASSERT_OK(maycup_insert_astnode(&node[2], &ast, root,
                                    MAYCUP_ASTNODE_TYPE_PARAGRAPH),
              fail);
    ASSERT_OK(maycup_insert_astnode(&node[3], &ast, node[2],
                                    MAYCUP_ASTNODE_TYPE_TEXT),
              fail);
    ...
    ASSERT_OK(maycup_insert_astnode(&node[5], &ast, root,
                                    MAYCUP_ASTNODE_TYPE_HEADING),
              fail);
    ast.data[node[5]].heading.level = 3;
    ASSERT_OK(maycup_insert_astnode(&node[6], &ast, node[5],
                                    MAYCUP_ASTNODE_TYPE_TEXT),
              fail);
    ASSERT_OK(maycup_astnode_data_text_ctor(&ast.data[node[6]].text,
                                            ...
                                            MAYCUP_TEXTSTYLE_PLAIN, false),
              fail);
    ASSERT_OK(maycup_insert_astnode(&node[7], &ast, node[5],
                                    MAYCUP_ASTNODE_TYPE_TEXT),
              fail);
