...
TEST_CASE(filereader_dtor_fclose_fail) {
    ...
    ASSERT_OK(M2H_filereader_ctor(&fr, filename), fail);
    ASSERT_EQ(M2H_filereader_dtor(&fr), M2H_RESULT_ERRNO, fail);
...
TEST_CASE(filereader_get_char_normal) {
    ...
    while (true) {
        expected = fgetc(fp);
        if (swc) {
            ASSERT_OK(M2H_reader_get_char(&fr, &actual), fail);
...
TEST_CASE(filereader_get_char_illegal_arg) {
    ...
    ASSERT_OK(M2H_filereader_ctor(&fr, filename), fail);
    ASSERT_EQ(M2H_reader_get_char(NULL, NULL), M2H_RESULT_ILLEGAL_ARGUMENT,
              fail);
...
TEST_CASE(filereader_tell_normal) {
    ...
    ASSERT_OK(M2H_filereader_ctor(&fr, filename), fail);
    ASSERT_OK(M2H_reader_tell(&fr, &cur), fail);
...
TEST_CASE(filereader_tell_ftell_fail) {
    ...
    ASSERT_OK(M2H_filereader_ctor(&fr, filename), fail);
    ASSERT_EQ(M2H_reader_tell(&fr, &cur), M2H_RESULT_ERRNO, fail);
...
TEST_CASE(filereader_tell_illegal_arg) {
    ...
    ASSERT_OK(M2H_filereader_ctor(&fr, filename), fail);
    ASSERT_EQ(M2H_reader_tell(NULL, &cur), M2H_RESULT_ILLEGAL_ARGUMENT, fail);
    ASSERT_EQ(M2H_reader_tell(&fr, NULL), M2H_RESULT_ILLEGAL_ARGUMENT, fail);
    ASSERT_EQ(fr.base.tell(NULL, &cur), M2H_RESULT_ILLEGAL_ARGUMENT, fail);
    ASSERT_EQ(fr.base.tell((M2H_Reader *)&fr, NULL),
...
TEST_CASE(filereader_seek_normal) {
    ...
    ASSERT_OK(M2H_filereader_ctor(&fr, filename), fail);
    ASSERT_OK(M2H_reader_seek(&fr, 384), fail);
    ASSERT_OK(M2H_reader_get_char(&fr, &cur), fail);
    ASSERT_EQ(cur, 'u', fail);
    ASSERT_OK(M2H_reader_seek(&fr, 0), fail);
    ASSERT_OK(M2H_reader_get_char(&fr, &cur), fail);
...
TEST_CASE(filereader_seek_fseek_fail) {
    ...
    ASSERT_OK(M2H_filereader_ctor(&fr, filename), fail);
    ASSERT_EQ(M2H_reader_seek(&fr, 384), M2H_RESULT_ERRNO, fail);
...
TEST_CASE(filereader_seek_illegal_arg) {
    ...
    ASSERT_EQ(fr.base.seek(NULL, 0), M2H_RESULT_ILLEGAL_ARGUMENT, fail);
...
TEST_CASE(stringreader_ctor_illegal_arg) {
    ...
    ASSERT_EQ(M2H_stringreader_ctor(NULL, stringname, stringlen),
              M2H_RESULT_ILLEGAL_ARGUMENT, fail);
    ASSERT_EQ(M2H_stringreader_ctor(&sr, NULL, stringlen),
              M2H_RESULT_ILLEGAL_ARGUMENT, fail);
    ASSERT_EQ(M2H_stringreader_ctor(&sr, stringname, stringlen + 1),
              M2H_RESULT_ILLEGAL_ARGUMENT, fail);
...
TEST_CASE(stringreader_dtor_double) {
    ...
    ASSERT_OK(M2H_stringreader_ctor(&sr, stringname, stringlen), fail);
    ASSERT_OK(M2H_stringreader_dtor(&sr), fail);
    ASSERT_EQ(M2H_stringreader_dtor(&sr), M2H_RESULT_ILLEGAL_ARGUMENT, fail);
...
TEST_CASE(stringreader_dtor_illegal_arg) {
    ASSERT_EQ(M2H_stringreader_dtor(NULL), M2H_RESULT_ILLEGAL_ARGUMENT, fail);
    return TEST_RESULT_PASS;
fail:
    return TEST_RESULT_FAIL;
}
...
TEST_CASE(stringreader_get_char_normal) {
    ...
    ASSERT_OK(M2H_stringreader_ctor(&sr, stringname_input, stringlen), fail);

    while (true) {
        expected = *ptr;
        ++ptr;
        if (swc) {
            ASSERT_OK(M2H_reader_get_char(&sr, &actual), fail);
            ASSERT_EQ(actual, expected, fail);
        } else {
            ASSERT_OK(M2H_reader_get_char(&sr, NULL), fail);
        }
        if (expected == '\0') {
            break;
        }
...
TEST_CASE(stringreader_get_char_eof) {
    ...
    ASSERT_OK(M2H_stringreader_ctor(&sr, str, 3), fail);
    ASSERT_OK(M2H_reader_get_char(&sr, &res), fail);
...
TEST_CASE(stringreader_get_char_illegal_arg) {
    ...
    ASSERT_EQ(M2H_reader_get_char(NULL, NULL), M2H_RESULT_ILLEGAL_ARGUMENT,
              fail);
    ASSERT_EQ(sr.base.get_char(NULL, NULL), M2H_RESULT_ILLEGAL_ARGUMENT, fail);
...
TEST_CASE(stringreader_tell_normal) {
    ...
    ASSERT_OK(M2H_stringreader_ctor(&sr, stringname, stringlen), fail);
    ASSERT_OK(M2H_reader_tell(&sr, &cur), fail);
    ASSERT_EQ(cur, 0, fail);
    ASSERT_OK(M2H_reader_get_char(&sr, NULL), fail);
    ASSERT_OK(M2H_reader_tell(&sr, &cur), fail);
    ASSERT_EQ(cur, 1, fail);
...
TEST_CASE(stringreader_tell_illegal_arg) {
    ...
    ASSERT_OK(M2H_stringreader_ctor(&sr, stringname, stringlen), fail);
    ASSERT_EQ(M2H_reader_tell(NULL, &cur), M2H_RESULT_ILLEGAL_ARGUMENT, fail);
    ASSERT_EQ(M2H_reader_tell(&sr, NULL), M2H_RESULT_ILLEGAL_ARGUMENT, fail);
    ASSERT_EQ(sr.base.tell(NULL, &cur), M2H_RESULT_ILLEGAL_ARGUMENT, fail);
    ASSERT_EQ(sr.base.tell((M2H_Reader *)&sr, NULL),
              M2H_RESULT_ILLEGAL_ARGUMENT, fail);
...
TEST_CASE(stringreader_seek_normal) {
    ...
    ASSERT_OK(M2H_stringreader_ctor(&sr, stringname, stringlen), fail);
    ASSERT_OK(M2H_reader_seek(&sr, 384), fail);
    ASSERT_OK(M2H_reader_get_char(&sr, &cur), fail);
    ASSERT_EQ(cur, 'u', fail);
    ASSERT_OK(M2H_reader_seek(&sr, 0), fail);
    ASSERT_OK(M2H_reader_get_char(&sr, &cur), fail);
    ASSERT_EQ(cur, 'L', fail);
